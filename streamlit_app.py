...
def extract_images_to_md(pdf_path):
    """Extract images from PDF and return Markdown references."""
    doc = fitz.open(pdf_path)
    images_folder = os.path.join(os.path.dirname(pdf_path), "extracted_images")
    ...
    for page_num in range(len(doc)):
        ...
        for img_index, img in enumerate(images):
            ...
            md_images += f"![Image page {page_num + 1} - {img_index + 1}]({image_full_path})\n"
    doc.close()
    return md_images
...
def enterprise_extract_pdf(pdf_path):
    ...
    """
    try:
        # Example: load Adobe credentials from a local JSON file path
        # In your environment, you might read from environment variables or st.secrets
        adobe_creds_path = os.getenv("ADOBE_CREDENTIALS_PATH", "pdfservices-api-credentials.json")
        if not os.path.exists(adobe_creds_path):
            raise FileNotFoundError(f"Adobe credentials file not found: {adobe_creds_path}")
    ...
    if os.path.exists(figures_folder):
        ...
        for fig_file in sorted(os.listdir(figures_folder)):
            ...
            if os.path.isfile(fig_path):
                # Optional: upload to S3 or reference locally
                s3_url = upload_file_to_s3(fig_path)  # define / adapt
                if s3_url:
...
def upload_file_to_s3(file_path):
    """
    For demonstration: tries to upload the file to an S3 bucket if credentials/bucket are set.
...
def enterprise_extract_website(url):
    ...
    Returns Markdown string or None if something fails.
    """
    # You have a Diffbot API token in your code
    api_token = os.getenv("DIFFBOT_API_TOKEN", "fdc6e4839de2e6b1f6a94009aff6aab1")
    if not api_token or api_token == "YOUR_DIFFBOT_TOKEN":
        logging.warning("Diffbot token not set or invalid. Provide 'DIFFBOT_API_TOKEN' in env.")
        return None
        ...
        if st.button("Generate Markdown"):
            ...
            with st.spinner("Extracting content from PDF..."):
                if extraction_method == "Extract Using Open-Source Tool":
                    ...
                    if not md_result:
                        st.error("Enterprise PDF extraction failed. Check logs.")
                        return
        ...
        if st.button("Generate Markdown"):
            ...
            with st.spinner("Extracting content from website..."):
                if extraction_method == "Extract Using Open-Source Tool":
                    ...
                    if not md_result:
                        st.error("Enterprise website extraction failed. Check logs or your Diffbot token.")
                        return
